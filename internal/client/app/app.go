// Package app manages the lifecycle of the GophKeeper adaptor application,
...
// App represents the main application structure, containing the gRPC adaptor,
// TUI (text user interface), and methods for controlling the application's lifecycle.
type App struct {
	// grpc adaptor
	grpcClient *adaptor.GophKeeperClient
...
// Init initializes the application by setting up the gRPC adaptor and TUI components.
func (a *App) Init() {
	var err error
	//var err error
	...
	// grpc adaptor
	{
		a.grpcClient, err = adaptor.NewGophKeeperClient(
			conf.Conf.EnableTLS,
...
func (a *App) Exit() {
	...
	if err := a.redisClient.Close(); err != nil {
		slog.Error("Error closing Redis adaptor")
	} else {
		slog.Info("Redis adaptor closed")
	}
