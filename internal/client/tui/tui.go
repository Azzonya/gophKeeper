// Package tui provides a text-based user interface (TUI) for the GophKeeper adaptor,
...
import (
	...
	"gophKeeper/internal/client/adaptor"
	proto "gophKeeper/pkg/proto/gophkeeper"
)

// TUI represents the text-based user interface for the GophKeeper adaptor,
// handling the display and interaction logic for user registration, login,
// and data item management.
type TUI struct {
	adaptor *adaptor.GophKeeperClient
	cache   *redis.Client
	app     *tview.Application
}

// NewTUI creates a new TUI instance with the given gRPC adaptor, initializing
// the application and setting up the user interface.
func NewTUI(adaptor *adaptor.GophKeeperClient, rDB *redis.Client) *TUI {
	return &TUI{
		adaptor: adaptor,
		cache:   rDB,
...
func (t *TUI) register() {
	...
	form.
		...
		AddButton("Register", func() {
			...
			_, err := t.adaptor.Register(ctx, &proto.RegisterRequest{
				Username: username,
				Password: password,
			})
			if err != nil {
...
func (t *TUI) login() {
	...
	form.
		...
		AddButton("Login", func() {
			...
			resp, err := t.adaptor.Login(ctx, &proto.LoginRequest{
				Username: username,
				Password: password,
			})
			if err != nil {
			...
			t.adaptor.BearerToken = resp.Token
...
func (t *TUI) createData() {
	...
	form.
		...
		AddButton("Submit", func() {
			...
			ctx, cancel := t.adaptor.CreateContextWithMetadata(15 * time.Second)
			defer cancel()

			_, err = t.adaptor.CreateData(ctx, req)
			if err != nil {
	...
	form.
		...
		AddButton("Submit", func() {
			...
			if t.adaptor.ServerAvailable {
				...
				resp, err := t.adaptor.GetData(ctx, req)
				if err != nil {
...
func (t *TUI) listData() {
	...
	resp, err := t.adaptor.ListData(ctx, &emptypb.Empty{})
	if err != nil {
...
func (t *TUI) updateData() {
	...
	form.
		...
		AddButton("Submit", func() {
			...
			resp, err := t.adaptor.UpdateData(ctx, req)
			if err != nil {
...
func (t *TUI) deleteData() {
	...
	form.
		...
		AddButton("Submit", func() {
			...
			resp, err := t.adaptor.DeleteData(ctx, req)
			if err != nil {
