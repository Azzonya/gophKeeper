...
import (
	...
	"gophKeeper/internal/conf"
	authorizerServiceP "gophKeeper/internal/server/domain/auth/service"
	dataItemsServiceP "gophKeeper/internal/server/domain/dataitems/service"
	usersServiceP "gophKeeper/internal/server/domain/users/service"
	grpcHandler "gophKeeper/internal/server/handler/grpc"
	dataItemsUsecaseP "gophKeeper/internal/server/usecase/dataitems"
	usersUsecaseP "gophKeeper/internal/server/usecase/users"
	"gophKeeper/pkg/proto/gophkeeper"
	"net"
	"os/signal"

	dataItemsRepoPgP "gophKeeper/internal/server/domain/dataitems/repo/pg"
	dataItemsRepoS3P "gophKeeper/internal/server/domain/dataitems/repo/s3"
	usersRepoPgP "gophKeeper/internal/server/domain/users/repo/pg"
	"log/slog"
	"os"
)
...
// certificate, private key, and the adaptor's Certificate Authority (CA) certificate for mutual TLS.
...
func loadTLSCredentials() (credentials.TransportCredentials, error) {
	...
	if !certPool.AppendCertsFromPEM(pemClientCA) {
		return nil, fmt.Errorf("failed to append adaptor CA certificate")
	}
