...
// NewS3Repo initializes a new S3Repo instance, setting up the S3 adaptor and bucket.
// It returns an error if the adaptor creation or bucket setup fails.
func NewS3Repo(ctx context.Context, S3Endpoint, S3AccessKey, S3SecretKey, S3Bucket string) (*S3Repo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create MinIO adaptor: %v", err)
	}
